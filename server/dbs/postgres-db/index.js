...
const insertRestaurant = async (document, analyze, cb) => {
  ...
  }INSERT into restaurants(restaurant_name, site_url, phone_number, city, street, state_or_province, country, zip) VALUES($1, $2, $3, $4, $5, $6, $7, $8)`;
  try {
...
const selectRestaurantByID = async (restaurant_id, analyze, cb) => {
  ...
  try {
    const res = await pool.query(query, [restaurant_id]);
    cb(null, res);
  } catch (err) {
    cb(err, null);
  }
...
const insertUser = async (document, analyze, cb) => {
  ...
  const query = `${
    analyze ? "EXPLAIN ANALYZE " : ""
  }INSERT into users(user_url, user_name, user_review_count, user_friend_count, user_photo_count, user_elite_status, user_profile_image) VALUES($1, $2, $3, $4, $5, $6, $7)`;
  try {
    const res = await pool.query(query, [
      user_url,
      user_name,
...
const insertPhoto = async (document, analyze, cb) => {
  ...
  const query = `${
    analyze ? "EXPLAIN ANALYZE " : ""
  }INSERT into photos(restaurant_id, user_id, helpful_count, not_helpful_count, photo_url, caption, upload_date) VALUES($1, $2, $3, $4, $5, $6, $7)`;
  try {
    const res = await pool.query(query, [
      restaurant_id,
      user_id,
...
const selectPhotoByRestaurantID = async (restaurant_id, analyze, cb) => {
  const query = `${
    analyze ? "EXPLAIN ANALYZE " : ""
  }SELECT * from photos WHERE restaurant_id = $1`;
  try {
    const res = await pool.query(query, [restaurant_id]);
    cb(null, res);
  } catch (err) {
...
const deletePhotoByID = async (photo_id, analyze, cb) => {
  ...
  try {
    const res = await pool.query(query, [photo_id]);
    cb(null, res);
  } catch (err) {
    cb(err, null);
  }
